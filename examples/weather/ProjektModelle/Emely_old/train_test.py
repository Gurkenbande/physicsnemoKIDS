                ...
                for batch_i, graph in enumerate(dataloader_val):
                  ...
                  #  val_loss_term2_meter.update(float(loss_qmse.detach()), n=yp_m.shape[0])
                    ...
                    if batch_i == len(dataloader_val) - 1:
                        print(f"[Epoch {epoch}] Avg Val Loss: {val_loss_meter.avg:.4f}")
    ...
    def test(self, model, dataloader, args, accelerator=None):
        ...
        with torch.no_grad():    
            for graph in dataloader:
                ...
                y_pred_m = y_pred[train_mask]
                y_m = y[train_mask]

                loss = mse(y_pred_m, y_m)
                loss_meter.update(float(loss.detach()), n=y_pred_m.shape[0])
        ...
        print(f"\n===== TEST RESULTS =====")
        print(f"Test MSE Loss: {loss_meter.avg:.6f}")
