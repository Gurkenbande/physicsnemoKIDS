...
class GNN4CD_Model(nn.Module):
    ...
    def __init__(self,encoding_dim=128,n_input_features=28,h_hid=32,n_layers=2,high_in=4,low2high_out=64,high_out=64,n_output_vars=4):
        super().__init__()
        ...
        self.processor = geometric_nn.Sequential('x, edge_index', [
            ...
            (GATv2Conv(in_channels=low2high_out, out_channels=high_out, heads=2, dropout=0.2,
                    aggr='mean', add_self_loops=True, bias=True), 'x, edge_index -> x'),
            (geometric_nn.BatchNorm(high_out * 2), 'x -> x'),
            nn.ReLU(),
            (GATv2Conv(in_channels=high_out * 2, out_channels=high_out, heads=2, dropout=0.2,
                    aggr='mean', add_self_loops=True, bias=True), 'x, edge_index -> x'),
            (geometric_nn.BatchNorm(high_out * 2), 'x -> x'),
            nn.ReLU(),
            (GATv2Conv(in_channels=high_out * 2, out_channels=high_out, heads=1, dropout=0.0,
                    aggr='mean', add_self_loops=True, bias=True), 'x, edge_index -> x'),
            nn.ReLU(),
        ])
    ...
    def forward(self, data, inference=False):
        ...
        encod_low2high = self.downscaler((encod_low, data.x_dict['high']),data["low", "to", "high"].edge_index)
        encod_high = self.processor(encod_low2high,data.edge_index_dict[('high', 'within', 'high')])
        x_high = self.predictor(encod_high)

        return x_high if inference else x_high
