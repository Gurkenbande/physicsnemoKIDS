...
def _plot_validation_selector_grids(
    ...
    out["lr"] = _save_index_grid(
        lr_maps,
        idxs,
        f"Validation Selector (epoch={int(epoch_idx)}) - LR channel {int(lr_channel_idx)}",
        os.path.join(plots_dir, f"selector50_epoch_{int(epoch_idx):03d}_lr_ch{int(lr_channel_idx)}.png"),
        cmap=cmap,
        ncols=ncols,
        vmin=lr_min,
        vmax=lr_max,
    ...
    out["gt"] = _save_index_grid(
        gt_maps,
        idxs,
        f"Validation Selector (epoch={int(epoch_idx)}) - Ground Truth precip",
        os.path.join(plots_dir, f"selector50_epoch_{int(epoch_idx):03d}_gt_precip.png"),
        cmap=cmap,
        ncols=ncols,
        vmin=pmin,
        vmax=pmax,
    ...
    out["pred"] = _save_index_grid(
        pred_maps,
        idxs,
        f"Validation Selector (epoch={int(epoch_idx)}) - Output precip",
        os.path.join(plots_dir, f"selector50_epoch_{int(epoch_idx):03d}_pred_precip.png"),
        cmap=cmap,
        ncols=ncols,
        vmin=pmin,
        vmax=pmax,
    ...
    out["mask"] = _save_index_grid(
        mask_maps,
        idxs,
        f"Validation Selector (epoch={int(epoch_idx)}) - Feature Mask",
        os.path.join(plots_dir, f"selector50_epoch_{int(epoch_idx):03d}_mask.png"),
        cmap=cmap,
        ncols=ncols,
        vmin=0.0,
        vmax=1.0,
    ...
    if out.get("lr"):
        payload["viz/selector/lr_ch0"] = wandb.Image(out["lr"])
    if out.get("gt"):
        payload["viz/selector/gt_precip"] = wandb.Image(out["gt"])
    if out.get("pred"):
        payload["viz/selector/output_precip"] = wandb.Image(out["pred"])
    if out.get("mask"):
        payload["viz/selector/mask"] = wandb.Image(out["mask"])
    wandb.log(payload)
    return out
...
def _plot_epoch_gt_pred_mask(
    ...
    fig.suptitle(f"Epoch {int(epoch_idx)} - Precip Validation Panels", fontsize=14)

    out_path = os.path.join(
        plots_dir, f"epoch_{int(epoch_idx):03d}_precip_lr_gt_out_mask.png"
    )
    plt.savefig(out_path, dpi=150, bbox_inches="tight")
    wandb.log(
        {
            "val_epoch": int(epoch_idx),
            "viz/epoch_precip_panel": wandb.Image(out_path),
        }
    )
    plt.close(fig)
    return out_path
    ...
    for epoch in range(epochs):
        ...
        if (
            ...
            for batch_idx, test_data in enumerate(test_loader, start=1):
                ...
                for b in range(E_batch.shape[0]):
                    ...
                    E_metric = _select_metric_channel(E_img, metric_channel_idx)
                    H_metric = _select_metric_channel(H_img, metric_channel_idx)

                    current_psnr, current_mae = util.calculate_score(
                        E_metric, H_metric, border=border
                    )
                    current_ssim = util.calculate_ssim(E_metric, H_metric, border=border)
                    diff = E_metric.astype(np.float64) - H_metric.astype(np.float64)
                    current_mse = float(np.mean(diff ** 2))
            ...
            avg_mae = avg_mae / mae_count if mae_count > 0 else float("nan")
            avg_ssim = avg_ssim / ssim_count if ssim_count > 0 else float("nan")
            avg_mse = avg_mse / mse_count if mse_count > 0 else float("nan")
            avg_rmse = avg_rmse / mse_count if mse_count > 0 else float("nan")
            ...
            if opt["rank"] == 0 and run_selector:
                ...
                try:
                    selector_paths = _plot_validation_selector_grids(
                        ...
                        plots_dir=plots_dir,
                        epoch_idx=epoch,
                        channel_idx=metric_channel_idx,
        ...
        if opt["rank"] == 0 and opt["train"].get("plot_epoch_samples", True):
            ...
            try:
                ...
                if saved_plot_path:
                    logger.info(f"Epoch {epoch + 1}: saved epoch plot to {saved_plot_path}")
            except Exception as e:
                logger.exception(
