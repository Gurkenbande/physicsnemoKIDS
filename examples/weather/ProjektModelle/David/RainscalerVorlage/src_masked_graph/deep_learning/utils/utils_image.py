...
# --------------------------------------------
'''
def imsave_plt(img, img_path):
    img = np.squeeze(img)
    if img.ndim == 3:
        img = img[:, :, [2, 1, 0]]
    #img = img [501:702,300:501]
    fig, ax = plt.subplots()
    #img = (img-np.amin(img)) / (np.amax(img)-np.amin(img))
    img = np.clip(img, 0,1)
    c = ax.imshow(np.log(img*140+0.01),vmin=-4.5, vmax=3.5)
    #c = ax.imshow(img)
    fig.tight_layout()
...
def imsave_plt(img, img_path):
    ...
    fig, ax = plt.subplots()
    img = np.clip(img, 0,1)
    c = ax.imshow(np.log(img*140+0.01))
...
# --------------------------------------------
# get np.float32  image of size HxWxn_channles (RGB)
# --------------------------------------------
def imread_float32(path, n_channels=3):
    #  input: path
    # output: HxWx3(RGB or GGG), or HxWx1 (G)
    img = np.load(path)
...
# --------------------------------------------
def float2single(img):
    img[img<0.357] == 0
    return np.float32(img / 140.0)
...
def tensor2uint_regression(img):
...
# PSNR  and mae
# --------------------------------------------
def calculate_score(img1, img2, border=0):
    ...
    return 10 * math.log10(1.0 / math.sqrt(mse)), mae
