...
class graph_constructor(nn.Module):
    ...
    def forward(self, idx, mssk, static_feat):
...
class MSRResNet0(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        m = torch.where(mask >= 0.01, 1, 0) #c

        
        h_m = torch.where(mask >= 0.01, 1, 0) #c #we should also think about strong underestimation in LR! >0.01 and < 100?
        ...
        adj = self.gc(self.idx, h, static_feat = static_feature) #c  # this one will be removed later, only exist in model_plain for backpropogation, or use fullA during testing
