...
from models.constraints import MultDownscaleConstraints
...
def window_partition(x, window_size):
    ...
    #[高和宽方向的窗口个数乘以 batch 个数，窗口大小，窗口大小，通道数] 的张量
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.window_size = window_size  # Wh, Ww # 窗口的高 Wh,宽 Ww
        ...
        coords = torch.stack(torch.meshgrid([coords_h, coords_w]))  # 2, Wh, Ww # 坐标，结构为 [2, Wh, Ww]
        coords_flatten = torch.flatten(coords, 1)  # 2, Wh*Ww # 重构张量结构为 [2, Wh*Ww]
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        img_size = to_2tuple(img_size)# 图像的大小
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
                 ...
                 use_checkpoint=False, upscale=2, img_range=1., upsampler='', resi_connection='1conv', constraints="none",
                 **kwargs):
        ...
        self.window_size = window_size #attention window size
        ...
        self.num_layers = len(depths) #swin transformer layer number
        self.embed_dim = embed_dim
        self.ape = ape #absolte position embedding, default is False
        self.patch_norm = patch_norm # normalization after patch embedding
        self.num_features = embed_dim
        self.mlp_ratio = mlp_ratio # mlp hidden channel/embedding channel
        ...
        patches_resolution = self.patch_embed.patches_resolution #resolution of the patch
        self.patches_resolution = patches_resolution
        ...
        if self.ape:
            # [1, number of patches, #embedding channels]
            self.absolute_pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
            trunc_normal_(self.absolute_pos_embed, std=.02) # truncate the normal distribution at std=0.02

        self.pos_drop = nn.Dropout(p=drop_rate)

        # stochastic depth 随机深度衰减规律，默认为[0， 0.1] 进行24等分后的而列表
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule

        # build Residual Swin Transformer blocks (RSTB)
        self.layers = nn.ModuleList() #multiple RSTB
        for i_layer in range(self.num_layers):
            #instantiate the RSTB
            ...
            self.layers.append(layer) #将RSTB对象插入ModuleList中
        ...
        if constraints == 'mult':
            self.constraints = MultDownscaleConstraints(upsampling_factor = self.upscale)
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear): # 判断是否为线性 Linear 层
            ...
            if isinstance(m, nn.Linear) and m.bias is not None: # 如果设置了偏置
