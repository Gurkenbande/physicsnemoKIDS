...
def define_M(opt):
    ...
    if net_type == 'unet':
        ...
        netM = UNet(opt_net['in_nc'],opt_net['out_nc'])
  # No need to train, but need BP to input
    if opt['is_train']:
        init_weights(netM,
...
def define_G(opt):
    ...
    # ----------------------------------------
        ...
        from models.models_srcnn import SRCNN as net
        netG = net(num_channels=opt_net['in_chans'],
                   constraints=opt_net['constraint'],
                   upsampling_factor=opt_net['upscale'])
    # ----------------------------------------
    # FFDNet
    # ----------------------------------------
    ...
    # ----------------------------------------
        ...
        netG = net(in_nc=opt_net['in_nc'],
                   ...
                   act_mode=opt_net['act_mode'],
                   upsample_mode=opt_net['upsample_mode'])
    elif net_type == 'msrresnet0_basic':
        from models.network_msrresnet_basic import MSRResNet0 as net
        netG = net(in_nc=opt_net['in_nc'],
                   out_nc=opt_net['out_nc'],
                   nc=opt_net['nc'],
                   nb=opt_net['nb'],
                   ...
                   act_mode=opt_net['act_mode'],
                   upsample_mode=opt_net['upsample_mode'])
    elif net_type == 'msrresnet0_mask':
        from models.network_msrresnet_mask import MSRResNet0 as net
        netG = net(in_nc=opt_net['in_nc'],
                out_nc=opt_net['out_nc'],
                nc=opt_net['nc'],
                nb=opt_net['nb'],
