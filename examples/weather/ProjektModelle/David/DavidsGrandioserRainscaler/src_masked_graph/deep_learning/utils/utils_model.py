...
def test_mode(model, L, mask=None, mode=0, refield=32, min_size=256, sf=1, modulo=1):
    '''
    # ---------------------------------------
    # Kai Zhang (github: https://github.com/cszn)
    # 03/Mar/2019
    ...
    if mode == 0:
        ...
        E = test_clip(L, model, sf)
    return E


def test_clip(img_lq, model, sf, x=32, tile_overlap=16):
    ...
    for h_idx in h_idx_list:
        for w_idx in w_idx_list:
            ...
            #in_patch_mask = mask[..., h_idx:h_idx + tile, w_idx:w_idx + tile]
            #out_patch, supervised_nodes = model(in_patch, in_patch_mask)
            #with torch.no_grad():
            out_patch, _ , _ = model(in_patch)
...
if __name__ == '__main__':
    ...
    class Net(torch.nn.Module):
        def __init__(self, in_channels=3, out_channels=3):
            ...
            self.conv = torch.nn.Linear(200, 1)
    ...
    with torch.no_grad():
        for mode in range(5):
            y = test_mode(model, x, mode, refield=32, min_size=64, sf=3, modulo=1)
            print(y.shape)
