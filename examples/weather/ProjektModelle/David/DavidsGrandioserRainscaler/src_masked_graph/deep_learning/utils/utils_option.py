...
def parse(opt_path, is_train=True):
    ...
    opt['path']['tensorboard'] = os.path.join(path_task, 'logs')
    ...
    #opt['netG']['constraint'] = opt['netG']['constraint'] if 'constraint' in opt else 'none'
    ...
    if 'M_optimizer_type' not in opt['train']:
        opt['train']['M_optimizer_type'] = "adam"
    if 'M_optimizer_betas' not in opt['train']:
        opt['train']['M_optimizer_betas'] = [0.9,0.999]
    if 'M_scheduler_restart_weights' not in opt['train']:
        opt['train']['M_scheduler_restart_weights'] = 1
    if 'M_optimizer_wd' not in opt['train']:
        opt['train']['M_optimizer_wd'] = 0
    if 'M_optimizer_reuse' not in opt['train']:
        opt['train']['M_optimizer_reuse'] = False
